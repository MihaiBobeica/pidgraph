...
def render_markdown(
    ...
    *,
        ...
        for finding in sorted(issues, key=lambda f: (_SEVERITY_ORDER[f.severity], f.check)):
            ...
            if finding.graph_incomplete:
                add(
                    ...
                    "document has a defect, or it could just mean extraction missed it — we "
                    "cannot tell which, so the severity is capped."
                )
            add("")
            add(finding.detail)
            add("")
    ...
    add("")
    add(
        "*Generated by deterministic rules. Run it again on the same inputs and you get the same "
