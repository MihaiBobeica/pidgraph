...
The ISA letter tables and DEXPI class names the parser actually uses live in `pidgraph/standards/`. The 2009 `Z` (Safety Instrumented System) is overlaid there as a labelled delta, and the 1984 `M` is kept because the drawings we read still use it. The citations behind those tables — including the `S`-means-Safety trap, the industry `DPI`/`PDI` split, and the DEXPI class names — are in [`assumptions.md`](../assumptions.md).
