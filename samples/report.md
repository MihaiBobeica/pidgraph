...
- ⓘ This finding rests on something *not* being found. That could mean the document has a defect, or it could just mean extraction missed it — we cannot tell which, so the severity is capped.

The procedure sets limits for this item, but we found nothing on the drawing to compare them against. That may well be our own extraction gap rather than a problem with the document — we read an estimated 12% of it — so this is capped in severity and reported as unresolved, not as a conflict.
...
Some symbols really are ambiguous without the drawing's legend sheet — the standard itself gives one shape two different meanings. Rather than guess, we list them here so a reader with the legend can settle them.
